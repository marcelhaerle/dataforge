...
export default function Dashboard() {
  ...
  const deleteDatabase = async (name: string) => {
    if (!confirm(`Willst du die Datenbank "${name}" wirklich löschen? Alle Daten gehen verloren!`)) return;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-4 flex justify-between items-center sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold tracking-tight text-slate-800">DataForge <span className="text-indigo-600">Console</span></h1>
        </div>
        <button
      ...
      <main className="max-w-7xl mx-auto p-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={fetchDatabases} className="p-2 text-slate-400 hover:text-indigo-600 transition-colors" title="Refresh">
        ...
        {!loading && databases.length === 0 && (
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
            ...
            <p className="text-slate-500 mb-6">Start by creating your first managed database instance.</p>
            <button
              onClick={() => setIsCreateModalOpen(true)}
      ...
      {isCreateModalOpen && (
        <DBCreateModal onClose={closeModal} />
      )}
    </div>
  );
}
